...
export interface addsingleAssignmentToexist {
  type: typeof addAssignment;
  payload: addassignment;
}
export interface deleteassignmentInter {
  type:string;
...
export type Adminaction =
  ...
  | errorinter
  | getteacherinter
  | getenrolledStudenstinter
  | allraisedRequestInter
  | allassignmentInter
  | addsingleAssignmentToexist
  | deleteassignmentInter
  | editAfterAdminPaymentInter| cancelRequestType
...
let allTeaherType = (
  data: singleTeacherOrAdmin[],
  dataTypeAdminOrTeacher: string
): getteacherinter => {
  return {
...
export let getallusers = (): any => async (dispatch: AppDispatch) => {
  ...
  try {
    ...
    let response = await axios.get(`http://localhost:8080/user/allusers`, {
      headers: {
...
export let addnewoneToorganisation =
  (data: any): any =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingType());
    try {
...
export let GetAllTeacheresFromServer =
  (role: String): any =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingType());
    try {
      ...
      let response = await axios.get(
        `http://localhost:8080/admin/all?role=${role}`,
...
export let GetAllEnrolledStudents =
  (): any => async (dispatch: AppDispatch) => {
    dispatch(loadingType());
    try {
      let token = Cookies.get("SchooleManagementAdminToken");
      let response = await axios.get(
        `http://localhost:8080/student/EnrolledAllStudents`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
...
export let GetAllRaisedRequests = (): any => async (dispatch: AppDispatch) => {
  ...
  try {
    let token = Cookies.get("SchooleManagementAdminToken");
    let response = await axios.get(`http://localhost:8080/request/all`, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
export let AddAssignmenttoStudents =
  (data: AssignmentFormValues): any =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingType());
    try {
      let token = Cookies.get("SchooleManagementAdminToken");
      ...
      dispatch({ type: addAssignment, payload: response.data.data });
      return response;
    } catch (error) {
      return error;
    }
...
export let UpdatedataForRequests =
  (data: any): any =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingType());
    try {
      ...
      let response = await axios.patch(
        `http://localhost:8080/request/update/${data.reqid}`,
        data,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      // dispatch(allRaisedRequest(response.data.requests))
...
export let GetAllAssignmentsForAdmin =
  (data: any): any =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingType());
    try {
      let token = Cookies.get("SchooleManagementAdminToken");
      let response = await axios.get(
        `http://localhost:8080/assignment/allAssinmentsForAdmin?role=${data.role}&student_class=${data.class}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
