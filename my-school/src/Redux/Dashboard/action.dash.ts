...
import {
  ...
  GETSUDENTS,
  LOADING,
  UPDATEAFETERFEEPAYMENT,
  UPDATETHEREOPENREQUEST,
} from "./types.dash";
...
export interface updateAfterfeepaymentInetr {
  type: typeof UPDATEAFETERFEEPAYMENT;
  payload: studentData;
}
export type userAction =
  ...
  | getrequestinter
  | addrequestinter
  | updatewithReopenRequestInter
  | updateAfterfeepaymentInetr;
...
export let getAllStudentsOfusers = (): any => async (dispatch: AppDispatch) => {
  ...
  return await axios
    .get(`http://localhost:8080/student/allStudents`, {
...
export let addNewRequest =
  (data: any): any =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingtype());
    let token = Cookies.get("SchooleManagementUserToken");
    return await axios.post(`http://localhost:8080/request/add`, data, {
      headers: { Authorization: `Bearer ${token}` },
    });
    //   .then((res)=>{
...
export let getAllStudentsRequests =
  (studentID: string): any =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingtype());
    let token = Cookies.get("SchooleManagementUserToken");
    let res = await axios.get(
      `http://localhost:8080/request/all/${studentID}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
